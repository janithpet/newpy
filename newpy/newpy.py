...
[
    parser.add_argument(
        *argument["name"], **{k: v for k, v in argument.items() if k != "name"}
    )
                ...
                if manager == "pipenv":
                    result = subprocess.run(
                        [
                            "bash",
                            "-c",
                            "source $(pipenv --venv)/bin/activate && pipenv install -e . && exit",
                        ],
                        capture_output=True,
                        text=True,
                    )
                    ...
                    result = subprocess.run(
                        [
                            "bash",
                            "-c",
                            f"source {_venv_name}/bin/activate && pip install -e . && exit",
                        ],
                        capture_output=True,
                        text=True,
                    )
                    ...
                    result = subprocess.run(
                        ["pip install -e ."], shell=True, capture_output=True, text=True
                    )
            # else:
            ...
            if arguments.precommit:
                ...
                if manager == "pipenv":
                    ...
                    _venv_name = (
                        current_dir_name
                        + "."
                        + (arguments.venv_name if arguments.venv_name else project_name)
                    )
                    result = subprocess.run(
